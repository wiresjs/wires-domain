...
module.exports = {
	 ...
	 * @return {[type]}
	 */
	invoke: function(avialableService, target, callReady) {
		...
		_.each(services, function(s, name) {
			avialableService[name] = s;
		});
		//	console.log(variables, Object.keys(avialableService))

		for (var i in variables) {
			var variableName = variables[i];
			if (!avialableService[variableName]) {
				return callReady(new domain.errors.Exception("Service with name '" + variableName + "'' was not found", 400), null);
			}
			args.push(avialableService[variableName]);
		}
		var results = [];

		async.eachSeries(args, function(argService, next) {
			if (_.isFunction(argService)) {
				var serviceResult = self.invoke(avialableService, argService, function(err, r) {
					results.push(r);
					next(err);
				});
			} else {
